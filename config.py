...
class Config:
    ...
    def set_parameters(self):
        ...
        # new codes:
        parser.add_argument('--algorithm', choices=['rf', 'adaline', 'linreg', 'mlp'],
                            default='rf',
                            help='Choose learner: random-forest (default), Adaline, or linear regression')
        parser.add_argument('--logepochs', action='store_true',
                            help='Print loss after every epoch (Adaline / linreg only)')
        parser.add_argument('--epochs', type=int, default=50,
                            help='Number of training epochs for gradient learners')
        parser.add_argument('--featuredump', action='store_true',
                            help='After reading data, dump the first feature vector and exit')
        parser.add_argument('--irregular_threshold', type=float, default=0.80,
                            help='Confidence (0-1) below which we treat a prediction as irregular')
        parser.add_argument('--irregular_action', choices=['nurse', 'police'], default='nurse',
                            help='Who to notify if irregular behaviour is detected')
        ...
        self.logepochs          = args.logepochs
        self.epochs             = args.epochs
        self.featuredump        = args.featuredump
        self.irregular_threshold= args.irregular_threshold
        self.irregular_action   = args.irregular_action
        # --------------------------------
